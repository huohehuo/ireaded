...
public class CircularProgressButton extends Button {
    ...
    private int changeProgressColor=getColor(R.color.circle_progress_five);
    ...
    private void init(Context context, AttributeSet attributeSet) {
        ...
        setProgressColor(changeProgressColor);
        initAttributes(context, attributeSet);

        mMaxProgress = 100;      //设置最大进度值
        mState = State.IDLE;    //初始化为闲置状态
        mStateManager = new StateManager(this);

        setText(mIdleText); //设置闲置状态的文字（可在xml直接设置）
//        setIcon(mIconError);
...
//                setText(mIdleText);
    ...
    public void setProgress(int progress) {
        ...
        if (mProgress >= mMaxProgress) {    //100时
            if (mState == State.PROGRESS) {
...
//                invalidate();
//            }
        } else if (mProgress > IDLE_STATE_PROGRESS) { //0时
        ...
        } else if (mProgress == ERROR_STATE_PROGRESS) { //-1时
            if (mState == State.PROGRESS) {
    ...
    public void setProgressColor(int color){
        this.changeProgressColor = color;
    }
}
